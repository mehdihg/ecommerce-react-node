...
export const reducerCart=(state=initialState,action)=>{
    switch(action.type){
        case AddToCart:{
            ...
            const updatedCartIndex= updatedCart.findIndex(item=>item.id === action.payload.id )
            ...
            case RemoveFromCart:{
                ...
                if(updateQuantity.quantity ===1){
                   ...
                   return{...state,cart:filteredCart,total:productTotal.total,totalPrice:state.totalPrice - action.payload.price,}
                }else{
                    updateQuantity.quantity--
                    updatedCart[updatedCartIndex]=updateQuantity
                    return{...state,cart:updatedCart,total:productTotal.total,totalPrice:state.totalPrice - action.payload.price,}
                }
